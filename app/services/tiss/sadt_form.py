"""
TISS SP/SADT Form Service
Handles SP/SADT guide form creation, validation, and XML generation
"""
...
from app.models import Invoice, Appointment, Clinic, User
...
class SADTFormService:
    """Service for managing SP/SADT guides"""
    ...
    async def create_sadt_guide(
        ...
        logger.info(f"Created SP/SADT guide {guide.id} for invoice {invoice_id}")
        return guide
    
    async def validate_sadt_guide(self, guide_id: int) -> Dict[str, any]:
        ...
        # Validate SADT data
        sadt_procedimentos = guide.sadt_data.get("procedimentos", [])
        for proc in sadt_procedimentos:
            codigo = proc.get("codigo_procedimento")
            tabela = proc.get("codigo_tabela", "06")  # SADT table
    ...
    async def generate_xml(self, guide_id: int) -> str:
        ...
        # Simplified XML generation (full implementation would use proper TISS structure)
...
<ans:mensagemTISS xmlns:ans="http://www.ans.gov.br/padroes/tiss/schemas">
    ...
    async def _calculate_integrity_hash(self, guide: TISSSADTGuide) -> str:
